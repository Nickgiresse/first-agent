...
public class DocumentServiceImpl implements DocumentService {
    ...
    @Override
    @Transactional
    public DocumentUploadResponse uploadDocument(String sessionToken, DocumentType documentType, MultipartFile file) {
        OnboardingSession session = onboardingSessionService.getValidSession(sessionToken);

        validateFile(file);

        Optional<StagingDocument> existing =
            stagingDocumentRepository.findByOnboardingSession_IdAndDocumentType(session.getId(), documentType);

        String filePath = storageService.store(file, session.getId().toString());

        StagingDocument document = existing.orElseGet(
            () -> StagingDocument.builder().onboardingSession(session).documentType(documentType).build()
        );
    ...
    public boolean hasAllRequiredDocuments(String sessionToken) {
        OnboardingSession session = onboardingSessionService.getValidSession(sessionToken);
        List<StagingDocument> documents = stagingDocumentRepository.findByOnboardingSession_Id(session.getId());
        Set<DocumentType> uploadedTypes = documents.stream()
            .map(StagingDocument::getDocumentType)
