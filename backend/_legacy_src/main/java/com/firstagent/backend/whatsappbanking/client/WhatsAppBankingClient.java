...
/**
 * Client vers l'API machine-à-machine (M2M) du WhatsApp banking (dépôt firstagent-demo),
 * qui est la SOURCE DE VÉRITÉ du client final. Ce microservice ne touche jamais la base du
 * WhatsApp banking directement : il passe par cette API à clé (en-tête X-API-Key), ce qui
 * préserve la logique métier, le journal d'audit scellé et l'unicité de la source de vérité.
 *
 * Contrat côté WhatsApp banking (gateway/onboarding_api.py) :
 *   POST /api/onboarding/verify-token  {token}                       -> valide sans consommer
 *   GET  /api/onboarding/account?phone=&rib=                         -> éligibilité / identité CBS
 *   POST /api/onboarding/customer      {token, name, account_number, -> écrit/MAJ, consomme le token
 *                                       pin, decision, kyc_*, cgu_accepted, source_ip}
 */
@Component
@Slf4j
public class WhatsAppBankingClient {
    ...
    public Map<String, Object> readAccount(String phone, String rib) {
        ...
        String url = UriComponentsBuilder.fromUriString(baseUrl + "/api/onboarding/account")
    ...
    public Map<String, Object> upsertCustomer(
        String token, String name, String accountNumber, String pin,
        String decision, Double kycScore, String kycCniNumber, String kycDocType,
        boolean cguAccepted, String sourceIp
    ) {
        Map<String, Object> body = new HashMap<>();
        body.put("token", token);
        body.put("name", name);
    ...
    private Map<String, Object> postJson(String path, Map<String, Object> body) {
        try {
            ...
            return restTemplate.exchange(baseUrl + path, HttpMethod.POST, request,
                new ParameterizedTypeReference<Map<String, Object>>() {}).getBody();
        } catch (RestClientException e) {
            throw translateError(e);
        }
    ...
    private BusinessException translateError(RestClientException e) {
        if (e instanceof HttpStatusCodeException httpError) {
            ...
            if (detail != null) {
                log.warn("Le WhatsApp banking a refusé la requête ({}) : {}", httpError.getStatusCode(), detail);
                return new BusinessException(detail);
            }
        }
        log.error("Erreur de communication avec le WhatsApp banking : {}", e.getMessage());
        return new BusinessException(
            "Erreur de communication avec le service bancaire. Vérifiez qu'il est bien démarré : " + e.getMessage()
        );
    }

    /**
     * L'API M2M renvoie ses erreurs sous {"detail": {"message": "..."}} (FastAPI HTTPException).
     * Extraction sans dépendance Jackson : le backend tourne sur Jackson 3 (tools.jackson) et
     * lier ce client à une version précise le rendrait fragile aux montées de version.
     */
