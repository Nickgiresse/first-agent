...
public class PythonVisionClient {
    ...
    public LivenessChallengeVerify verifyLivenessChallenge(String sessionId, String action, List<byte[]> frames) {
        MultiValueMap<String, Object> body = new LinkedMultiValueMap<>();
    ...
    private <T> T post(String path, MultiValueMap<String, Object> body, ParameterizedTypeReference<T> responseType) {
        try {
            HttpHeaders headers = new HttpHeaders();
            ...
            return restTemplate.exchange(baseUrl + path, HttpMethod.POST, request, responseType).getBody();
        } catch (RestClientException e) {
            throw translateError(e);
        }
    }
    ...
    private BusinessException translateError(RestClientException e) {
        if (e instanceof HttpStatusCodeException httpError) {
            ...
            if (detail != null) {
                log.warn("Le microservice de vision a refusé la requête ({}) : {}", httpError.getStatusCode(), detail);
