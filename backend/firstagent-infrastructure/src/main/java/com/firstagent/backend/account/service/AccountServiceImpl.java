...
public class AccountServiceImpl implements AccountService {
   ...
   *
   * <p>Le référentiel des comptes est celui du WhatsApp banking (source de vérité) : on l'interroge
   * via l'API machine-à-machine plutôt que de dupliquer l'information ici. La table locale
   * bank_accounts n'est qu'un miroir technique, nécessaire parce que la session d'onboarding s'y
   * rattache par clé étrangère ; elle est alimentée/rafraîchie à partir de la réponse de la banque.
   * Si la banque est injoignable, on se rabat sur ce miroir afin qu'une coupure du bot n'interrompe
   * pas un parcours déjà commencé.
   */
  @Override
  @Transactional
  public AccountVerificationResponse verifyAccount(AccountVerificationRequest request) {
    ...
    BankAccount bankAccount =
        resolveFromWhatsAppBanking(accountNumber)
            .orElseGet(
                () ->
                        ...
                        .orElseThrow(
                            () ->
                                new AccountNotFoundException(
                                    "Aucun compte bancaire trouvé avec ce numéro")));
   ...
   *
   * <p>Un référentiel qui ne connaît aucun téléphone pour ce compte ne permet pas de conclure.
   * Laisser passer reviendrait à désactiver le contrôle dès qu'une donnée manque, ce qui en ferait
   * une protection illusoire : le client est renvoyé en agence, où un conseiller peut vérifier son
   * identité.
   */
   ...
   *
   * <p>Les deux sources ne se ressemblent pas : le référentiel stocke souvent « 237 6 85 44 55 11 »
   * ou « +237685445511 », le bot transmet ce que lui donne WhatsApp. Comparer les chaînes brutes
   * ferait échouer le contrôle sur de simples espaces.
   */
  ...
  private java.util.Optional<BankAccount> resolveFromWhatsAppBanking(String accountNumber) {
    ...
    try {
      ...
      log.warn(
          "Référentiel bancaire injoignable pour {} : {}. Repli sur le miroir local.",
          accountNumber,
          e.getMessage());
    ...
    String firstName =
        fullName.contains(" ") ? fullName.substring(lastName.length()).trim() : fullName;
