...
class FaceVerificationControllerTest {
  ...
  @MockitoBean private FaceVerificationService faceVerificationService;
  ...
  void verifyReturnsOkWithMatchResult() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/documents/face-verification/verify")
  ...
  void getReturnsOkWithExistingResult() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/documents/face-verification").header("X-Session-Token", sessionToken))
        .andExpect(status().isOk())
