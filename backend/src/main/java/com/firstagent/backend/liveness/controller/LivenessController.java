...
public class LivenessController {
    ...
    public ResponseEntity<ApiResponse<ChallengeStartResponse>> start(@RequestHeader("X-Session-Token") String sessionToken) {
    ...
    public ResponseEntity<ApiResponse<ChallengeVerifyResponse>> verify(
        ...
        ChallengeVerifyResponse response = livenessService.verifyAction(sessionToken, action, frameBytes);
        return ResponseEntity.ok(ApiResponse.success(response));
    }

    @GetMapping("/challenge/status")
    public ResponseEntity<ApiResponse<ChallengeStatusResponse>> status(@RequestHeader("X-Session-Token") String sessionToken) {
    ...
    private byte[] readBytes(MultipartFile file) {
        try {
            ...
            throw new BusinessException("Impossible de lire une image de la rafale envoyée : " + e.getMessage());
        }
    }
}
