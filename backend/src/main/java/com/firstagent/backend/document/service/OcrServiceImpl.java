...
public class OcrServiceImpl implements OcrService {
    ...
    public OcrExtractionResponse extractDocumentData(String sessionToken) {
        ...
        StagingDocument front = stagingDocumentRepository.findByOnboardingSession_IdAndDocumentType(session.getId(), DocumentType.CNI_RECTO)
            .orElseThrow(() -> new BusinessException("Le recto de la CNI doit être téléversé avant l'extraction"));
        StagingDocument back = stagingDocumentRepository.findByOnboardingSession_IdAndDocumentType(session.getId(), DocumentType.CNI_VERSO)
            .orElseThrow(() -> new BusinessException("Le verso de la CNI doit être téléversé avant l'extraction"));
        ...
        StagingOcrResult ocrResult = stagingOcrResultRepository.findByOnboardingSession_Id(session.getId())
        ...
        if (lowConfidence) {
            throw new BusinessException(
                "La qualité de l'extraction est trop faible (confiance " + Math.round(result.confidenceScore())
    ...
    @Override
    @Transactional
    public OcrExtractionResponse confirmExtractedData(String sessionToken, OcrConfirmationRequest request) {
        OnboardingSession session = onboardingSessionService.getValidSession(sessionToken);
        StagingOcrResult ocrResult = findExistingResult(session.getId());

        if (ocrResult.getStatus() != OcrStatus.EXTRACTED && ocrResult.getStatus() != OcrStatus.CONFIRMED) {
            throw new BusinessException("Aucune extraction OCR valide à confirmer pour ce client. Relancez l'extraction.");
        }
        ...
        if (isCni) {
            ensureDocumentNumberMatchesExtraction(ocrResult.getDocumentNumber(), request.getDocumentNumber());
        }
        ensureMatchesAccountHolder(session.getBankAccount().getFirstName(), session.getBankAccount().getLastName(), request);
    ...
    public boolean isConfirmed(String sessionToken) {
        OnboardingSession session = onboardingSessionService.getValidSession(sessionToken);
        return stagingOcrResultRepository.existsByOnboardingSession_IdAndStatus(session.getId(), OcrStatus.CONFIRMED);
    }

    private void ensureNameMatchesExtraction(String fieldLabel, String extractedValue, String submittedValue) {
        if (!isUsable(extractedValue)) {
            return;
        }
        if (StringSimilarity.similarity(extractedValue, submittedValue) < ocrNameSimilarityThreshold) {
            throw new BusinessException(
                "Le " + fieldLabel + " saisi ne correspond pas à celui lu sur votre document. Corrigez la saisie ou reprenez les photos."
            );
        }
    }
    ...
    private void ensureDocumentNumberMatchesExtraction(String extractedValue, String submittedValue) {
        ...
        if (StringSimilarity.similarity(normalizedExtracted, normalizedSubmitted) < DOCUMENT_NUMBER_SIMILARITY_THRESHOLD) {
            throw new BusinessException("Le numéro de document saisi ne correspond pas à celui lu sur votre CNI.");
        }
    }

    private void ensureBirthDateMatchesExtraction(LocalDate extractedValue, LocalDate submittedValue) {
        if (extractedValue == null) {
            return;
        }
        if (!extractedValue.equals(submittedValue)) {
            throw new BusinessException("La date de naissance saisie ne correspond pas à celle lue sur votre document.");
        }
    }

    private void ensureMatchesAccountHolder(String accountFirstName, String accountLastName, OcrConfirmationRequest request) {
        double firstNameSimilarity = StringSimilarity.similarity(accountFirstName, request.getFirstName());
    ...
    private StagingOcrResult findExistingResult(UUID onboardingSessionId) {
        return stagingOcrResultRepository.findByOnboardingSession_Id(onboardingSessionId)
            .orElseThrow(() -> new ResourceNotFoundException("Aucune extraction OCR trouvée pour ce client"));
    }
