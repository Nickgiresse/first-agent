...
public interface StagingDocumentRepository extends JpaRepository<StagingDocument, UUID> {
    ...
    Optional<StagingDocument> findByOnboardingSession_IdAndDocumentType(UUID onboardingSessionId, DocumentType documentType);

    boolean existsByOnboardingSession_IdAndDocumentType(UUID onboardingSessionId, DocumentType documentType);
